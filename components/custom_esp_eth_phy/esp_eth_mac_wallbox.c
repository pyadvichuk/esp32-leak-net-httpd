...
#define ETH_ACTUAL_MALLOC_CAP       MALLOC_CAP_SPIRAM
                                    /*MALLOC_CAP_8BIT*/
...
static esp_err_t emac_wallbox_set_mediator(esp_eth_mac_t *mac, esp_eth_mediator_t *eth)
{
...
static esp_err_t emac_wallbox_write_phy_reg(esp_eth_mac_t *mac, uint32_t phy_addr, uint32_t phy_reg, uint32_t reg_value)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t emac_wallbox_read_phy_reg(esp_eth_mac_t *mac, uint32_t phy_addr, uint32_t phy_reg, uint32_t *reg_value)
{
    esp_err_t ret = ESP_OK;
...
static esp_err_t emac_wallbox_set_addr(esp_eth_mac_t *mac, uint8_t *addr)
{
    esp_err_t ret = ESP_OK;
...
static esp_err_t emac_wallbox_get_addr(esp_eth_mac_t *mac, uint8_t *addr)
{
    esp_err_t ret = ESP_OK;
    MAC_CHECK(addr, "can't set mac addr to null", err, ESP_ERR_INVALID_ARG);
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t emac_wallbox_set_link(esp_eth_mac_t *mac, eth_link_t link)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
    switch (link) {
...
static esp_err_t emac_wallbox_set_speed(esp_eth_mac_t *mac, eth_speed_t speed)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
    switch (speed) {
...
static esp_err_t emac_wallbox_set_duplex(esp_eth_mac_t *mac, eth_duplex_t duplex)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
    switch (duplex) {
...
static esp_err_t emac_wallbox_set_promiscuous(esp_eth_mac_t *mac, bool enable)
{
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t emac_wallbox_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
    ...
    MAC_CHECK(emac_hal_transmit_frame(&emac->hal, buf, length) == length,
              "Frame transmit error", err, ESP_ERR_INVALID_SIZE);
    return ESP_OK;
err:
    return ret;
}

static esp_err_t emac_wallbox_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
...
static void emac_wallbox_rx_task(void *arg)
{
    ...
    while (1) {
        ...
        do {
            ...
            buffer = heap_caps_malloc(length, ETH_ACTUAL_MALLOC_CAP);
            if (NULL == buffer) {
                ESP_LOGE(TAG, "can't allocate packet buffer (avail: %d, max_continuous: %d)",
                    heap_caps_get_free_size(ETH_ACTUAL_MALLOC_CAP),
                    heap_caps_get_largest_free_block(ETH_ACTUAL_MALLOC_CAP));
            } else if (emac_wallbox_receive(&emac->parent, buffer, &length) == ESP_OK) {
...
static void emac_wallbox_init_smi_gpio(emac_esp32_t *emac)
{
    /* Setup SMI MDC GPIO */
...
static esp_err_t emac_wallbox_init(esp_eth_mac_t *mac)
{
    esp_err_t ret = ESP_OK;
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
    ...
    emac_wallbox_init_smi_gpio(emac);
...
static esp_err_t emac_wallbox_deinit(esp_eth_mac_t *mac)
{
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
    esp_eth_mediator_t *eth = emac->eth;
#ifdef CONFIG_PM_ENABLE
...
static esp_err_t emac_wallbox_del(esp_eth_mac_t *mac)
{
    emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
...
IRAM_ATTR void emac_wallbox_isr_handler(void *args)
{
...
esp_eth_mac_t *esp_eth_mac_new_wallbox(const eth_mac_config_t *config)
{
    ...
    emac->parent.set_mediator = emac_wallbox_set_mediator;
    emac->parent.init = emac_wallbox_init;
    emac->parent.deinit = emac_wallbox_deinit;
    emac->parent.del = emac_wallbox_del;
    emac->parent.write_phy_reg = emac_wallbox_write_phy_reg;
    emac->parent.read_phy_reg = emac_wallbox_read_phy_reg;
    emac->parent.set_addr = emac_wallbox_set_addr;
    emac->parent.get_addr = emac_wallbox_get_addr;
    emac->parent.set_speed = emac_wallbox_set_speed;
    emac->parent.set_duplex = emac_wallbox_set_duplex;
    emac->parent.set_link = emac_wallbox_set_link;
    emac->parent.set_promiscuous = emac_wallbox_set_promiscuous;
    emac->parent.transmit = emac_wallbox_transmit;
    emac->parent.receive = emac_wallbox_receive;
    /* Interrupt configuration */
    MAC_CHECK(esp_intr_alloc(ETS_ETH_MAC_INTR_SOURCE, ESP_INTR_FLAG_IRAM, emac_wallbox_isr_handler,
...
#endif
    /* create rx task */
    BaseType_t xReturned = xTaskCreate(emac_wallbox_rx_task, "emac_rx", config->rx_task_stack_size, emac,
